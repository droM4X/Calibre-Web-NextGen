...
| #91 | [#1221](https://github.com/crocodilestick/Calibre-Web-Automated/pull/1221) | @EuanKerr | NewBookProcessor: route auto-send to a single user when a book is dropped under `/cwa-book-ingest/<username>/`. Tier `needs-review` — flagged behavior change for existing subfolder users (auto-send is now skipped when the first segment doesn't match a CWA username). | `TBD` | TBD |
...
| TBD | Cover-picker Kobo preview ([#84](https://github.com/new-usemame/Calibre-Web-NextGen/issues/84)): when `config_kobo_cover_padding_enabled` is on, the `/book/<id>/cover` page gets a "Kobo preview" panel with a toggle + mirrored aspect / fill-mode / custom-color controls. Toggling it on swaps every visible cover (current cover, candidate grid, confirm modal, URL-thumb) to a server-rendered padded variant via a new `POST /book/<id>/cover/kobo-preview` endpoint that reuses `cover_padding.pad_blob`. Settings on this panel are picker-session-local — they do not mutate global admin defaults. Per-image cache keyed on (aspect, fill_mode, color) so flipping back and forth doesn't re-fetch. New `render_kobo_preview_data_url()` helper in `cps/services/cover_padding.py`. 17 unit tests (5 helper tests + 12 structural endpoint/template tests). See `notes/cover-picker-kobo-preview-DESIGN.md`. | `TBD` | TBD |
